...
function initGlobe() {
  ...
  const texture = textureLoader.load('mapadosirelala.png'); // seu arquivo
  ...
  const cloudGeometry = new THREE.SphereGeometry(1.01, 64, 64); // raio ligeiramente maior
  const cloudTexture = textureLoader.load('nuvemdosirelala.png'); // textura com transparência
  const cloudMaterial = new THREE.MeshPhongMaterial({
    map: cloudTexture,
    transparent: true,   // permite transparência
    opacity: 0.8,        // ajuste de opacidade conforme desejado
